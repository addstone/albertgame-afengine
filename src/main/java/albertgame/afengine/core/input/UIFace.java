...
public class UIFace implements IHandler{    
    ...
    private boolean addUiInAllImpl(UIActor actor){
        if(!containsUiInAll(actor.getUiName())){
            ...
            DebugUtil.log("already has ui for "+actor.getUiName());
            return false;
        }
    }    
    ...
    public void addMsgUiMap(Long type,UIActor ui){
        ...
        if(uilist==null){
            ...
            uilist.add(ui);
            DebugUtil.log("add msg ui map:"+ui.getUiName()+" - "+type);
        }
        else{
            if(uilist.contains(ui)){
                DebugUtil.log("already has type map for "+ui.getUiName());
            }
            else{
                DebugUtil.log("add msg ui map:"+ui.getUiName()+" - "+type);
                uilist.add(ui);
    ...
    public void removeMsgUiMap(Long type,UIActor ui){
        List<UIActor> uilist=msgTypeUIMap.get(type);
        if(uilist==null){
            DebugUtil.log("no type of msg ui found,type:"+type);
            return;
        }
        if(uilist.contains(ui)){
            uilist.remove(ui);
            DebugUtil.log("remove ui map successfully");
        }else{
            DebugUtil.log("no ui msp");
        }
    }
    ...
    public boolean handle(Message msg){
        ...
        if(handList==null){
            DebugUtil.log("not hand for msg type:"+msg.msgType);
            return false;
        }

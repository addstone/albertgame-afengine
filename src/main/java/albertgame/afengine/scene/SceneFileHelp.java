...
public class SceneFileHelp {
    ...
    public static Scene loadSceneFromXML(Element sceneroot) {
        if (!sceneroot.getName().equals("scene")) {
            DebugUtil.log(DebugUtil.LogType.ERROR, "file is not a scene file.");
            return null;
        }
        ...
        if (actormap == null) {
            DebugUtil.log(DebugUtil.LogType.ERROR, "has no actor map of scene.");
        } else {
            //再创建实体父子映射
            List<Actor> rootactorList = orderMap(actorMap, actormap);
            rootactorList.forEach((actor) -> {
                DebugUtil.log(DebugUtil.LogType.ERROR, "put " + actor.getName());
    ...
    private static Map<String, Actor> loadAllActors(Element root) {
        ...
        if (actordata == null) {
            DebugUtil.log(DebugUtil.LogType.ERROR, "has no actor data of scene.");
        } else {
    ...
    private static List<Actor> orderMap(Map<String, Actor> actorMap, Element orderRoot) {
        ...
        while (eleiter.hasNext()) {
            ...
            if (actor == null) {
                DebugUtil.log(DebugUtil.LogType.ERROR, "actor for " + name + " is not defined.");
            } else {
                //将要添加到场景的根下
                rootlist.add(actor);
    ...
    private static void mapActortoParent(Map<String, Actor> actorMap, Element parent, Element child) {
        ...
        if (parent != null) {
            ...
            if (pactor == null) {
                DebugUtil.log(DebugUtil.LogType.ERROR, "parent actor - " + pname + " not found.");
            } else {
                Actor cactor = actorMap.get(child.getName());
                if (cactor == null) {
                    DebugUtil.log(DebugUtil.LogType.ERROR, "child actor - " + child.getName() + " not found.");
                } else {
                    DebugUtil.log(DebugUtil.LogType.ERROR, "actor: " + parent.getName() + " add node - " + child.getName());
    ...
    private static void makeParentMatch(Actor parent, Actor child, Map<String, Element> elementMap) {
        ...
        if (parent != null) {
            ...
            if (parente == null) {
                DebugUtil.log(DebugUtil.LogType.ERROR, "not found for parent element for child.");
            } else {
    ...
    public static void loadStaticActorFromXML(Element staticactorroot) {
        ...
        if (staticactorroot == null || !staticactorroot.getName().equals("staticactor-list")) {
            DebugUtil.log(DebugUtil.LogType.ERROR, "not a static actor list file.");
            return;
        }
    ...
    public static void outputStaticActorToXML(Element staticactorroot) {
        ...
        if (staticactorroot == null) {
            DebugUtil.log(DebugUtil.LogType.ERROR, "static actor root is null,output static actor list failed");
            return;
        }
        staticactorroot.setName("staticactor-list");
        staticactorroot.clearContent();
        Iterator<Actor> actorentry = Actor.staticActorList.iterator();
        while (actorentry.hasNext()) {
            Actor actor = actorentry.next();
    ...
    private static Element getModuleElement(String modulepath) {
        ...
        Element root = XmlUtil.readXMLFileDocument(filepath, false).getRootElement();
        if (root != null && !root.getName().equals("modlibs")) {
            DebugUtil.log(DebugUtil.LogType.ERROR, "modulepath:" + modulepath + " is not a mod lib. please check.");
            return null;
        }
    ...
    private static void createModuleComponent(Element module, Actor owner) {
        ...
        while (compiter.hasNext()) {
            ...
            ActorComponent comp = (ActorComponent) App.factorys.create("component-fac", compname, element, owner.valueMap);
            DebugUtil.assertNotNull(comp, DebugUtil.LogType.SEVER, compname + "create failed");
            DebugUtil.log(DebugUtil.LogType.ERROR, "component created successfully:" + comp.getComponentName());
            owner.addComponent(comp, false);
        }
        DebugUtil.log(DebugUtil.LogType.ERROR, "module created successfully:" + module.getName());
    }

    ...
    */
    public static Scene loadSceneFromXML(Element sceneroot) {
        if(!sceneroot.getName().equals("scene")){
            DebugUtil.log("file is not a scene file.");
            return null;
        }
        ...
        if(actormap==null){
            DebugUtil.log("has no actor map of scene.");
        }else{
        //再创建实体父子映射
            List<Actor> rootactorList = orderMap(actorMap,actormap);
            rootactorList.forEach((actor) -> {
                DebugUtil.log("put "+actor.getName());
                scene.rootList.add(actor);
            });
        }        
...
//        scene.awakeAllActors();
    ...
    private static Map<String,Actor> loadAllActors(Element root){
        ...
        if(actordata==null){
            DebugUtil.log("has no actor data of scene.");            
        }else{
    ...
    private static List<Actor> orderMap(Map<String,Actor> actorMap,Element orderRoot){
        ...
        while(eleiter.hasNext()){
            ...
            if(actor==null){
                DebugUtil.log("actor for "+name+" is not defined.");
                continue;
            }else{
                //将要添加到场景的根下
                rootlist.add(actor);
    ...
    private static void mapActortoParent(Map<String,Actor> actorMap,Element parent,Element child){
        ...
        if(parent!=null){
            ...
            if(pactor==null){
                DebugUtil.log("parent actor - "+pname+" not found.");
            }else{
                Actor cactor=actorMap.get(child.getName());
                if(cactor==null){
                    DebugUtil.log("child actor - "+child.getName()+" not found.");
                }else{
                    DebugUtil.log("actor: "+parent.getName()+ " add node - "+child.getName());
    ...
    */
    public static void outputSceneToXML(Scene scene,Element sceneroot){
        ...
        outputActors(scene.rootList,actordata);
        //创建实体父子映射
        Element actormap=sceneroot.addElement("actormap");
        makeParent(scene.rootList,actormap);
    }
    private static void outputActors(List<Actor> rootActors,Element actordata){
        Iterator<Actor> entryiter = rootActors.iterator();
        while(entryiter.hasNext()){
            Actor actorentry = entryiter.next();
            String actorname=actorentry.getName();
    ...
    private static void makeParent(List<Actor> rootActors,Element actormap){
        Iterator<Actor> entryiter = rootActors.iterator();
        ...
        Iterator<Actor> entryiter2 = rootActors.iterator();
        while(entryiter2.hasNext()){
            Actor actorentry = entryiter2.next();
            makeParentMatch(null,actorentry,elementMap);
        }        
    }
    private static void makeParentMatch(Actor parent,Actor child,Map<String,Element> elementMap){
        ...
        if(parent!=null){
            ...
            if(parente==null){
                DebugUtil.log("not found for parent element for child.");
            }else{
    ...
    */
    public static void loadStaticActorFromXML(Element staticactorroot) {
        ...
        if(staticactorroot==null||!staticactorroot.getName().equals("staticactor-list")){
            DebugUtil.log("not a static actor list file.");
            return;
        }
        ...
        while(eleiter.hasNext()){
            ...
            if(actor!=null){
                Actor.staticActorList.add(actor);
            }
    ...
    public static void outputStaticActorToXML(Element staticactorroot){
        ...
        if(staticactorroot==null){
            DebugUtil.log("static actor root is null,output static actor list failed");
            return;
        }
    ...
    */
    ...
    private static Element getModuleElement(String modulepath){
        ...
        if(root!=null&&!root.getName().equals("modlibs")){
            DebugUtil.log("modulepath:"+modulepath+" is not a mod lib. please check.");
            return null;
        }        
    ...
    private static void createModuleComponent(Element module,Actor owner){
        ...
        while(compiter.hasNext()){
            ...
            if(factory!=null){
                ...
                DebugUtil.log("component created successfully:"+comp.getComponentName());
                owner.addComponent(comp, false);
            }
        }
        DebugUtil.log("module created successfully:"+module.getName());
    }
